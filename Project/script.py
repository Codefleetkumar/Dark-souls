...
def pause_menu():
    ...
    while True:
        ...
        #Texts
        paused = font1.render("Paused", True, (0, 0, 0))
        ...
        screen.blit(paused, [screenwidth/2 - 150, screenheight/2 - 100])
        screen.blit(play_again, [screenwidth/2 - 60, screenheight/2 + 30])
        screen.blit(main_menu, [screenwidth/2 - 60, screenheight/2 + 60])
        screen.blit(_exit, [screenwidth/2 - 60, screenheight/2 + 90])
...
def highscore1():
    ...
    #c.execute('CREATE TABLE Score (Number REAL, Name TEXT)') 
          
         
    name = "score"
    c.execute("INSERT INTO Score (Number, Name) VALUES(?, ?)", (s.count, name)) 
    conn.commit() 

    sql_command1 = """select max(Number) from Score ;"""

    c.execute(sql_command1) 
    rows = c.fetchall()

    font = py.font.SysFont("Arial", 25)
    highscore = font.render("Highscore :" + str(rows), True, (0, 0, 0))
...
def game():
    ...
    #Music
    py.mixer.music.load('The Opened Way.mp3')
    py.mixer.music.play()
    ...
    while True:
        ...
        #def music():
         #   py.mixer.Sound.play(game_sound)
          #  py.mixer.music.stop()
        #music()
